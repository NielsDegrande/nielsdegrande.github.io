...
initialize_blog:
    ...
    --message "Add a blog entry to blog/index.html. Today's date is $(shell date +%Y-%m-%d). Topic is '$(ESCAPED_TOPIC)'."
    ...
    --message "Write a blog post about '$(ESCAPED_TOPIC)'. Look at other posts in the `blog` folder to get the style right. Here is the content to incorporate: '$(ESCAPED_CONTENT)'."
